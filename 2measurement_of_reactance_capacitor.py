...
RAW_HEADER = ["Frequency [Hz]", "1/f [1/Hz]", "repeat", "Vrms [V]", "Irms [A]", "XC [Ohm]"]
SUMMARY_HEADER = [
    "Frequency [Hz]",
    "1/f [1/Hz]",
    "Vrms_mean [V]",
...
def get_args():
    parser = argparse.ArgumentParser(description="リアクタンス測定(キャパシタ)")
...
def main(frequencies=None, vpp=None, n_repeat=None):
    ...
    raw_csv_name = f"reactance_capacitor_raw_{run_id}.csv"
    summary_csv_name = f"reactance_capacitor_summary_{run_id}.csv"
    ...
    try:
        ...
        for freq in frequencies:
            ...
            for repeat in range(1, N_REPEAT + 1):
                ...
                print(
                    ...
                    f"Vrms = {vrms:.5f} V, Irms = {irms:.5f} A, XC = {XC:.5f} Ω"
                )

                raw_row = [freq, 1.0 / freq, repeat, vrms, irms, XC]
            ...
            print(
                f"  mean ± std: XC = {xc_mean:.5f} ± {xc_std:.5f} Ω, "
        ...
        x = np.array([row[1] for row in summary_results], dtype=float)
        y = np.array([row[6] for row in summary_results], dtype=float)
        yerr = np.array([row[7] for row in summary_results], dtype=float)
        ...
        C = 1 / (2 * math.pi * a)
        u_C = abs(u_a / (2 * math.pi * a ** 2))
        x_fit = np.linspace(x.min(), x.max(), 300)
        y_fit = a * x_fit

        representative_lines = [
            f"C = {C * 1e6:.5f} ± {u_C * 1e6:.5f} μF",
        ...
        fig_path = image_path(f"reactance_capacitor_errorbar_fit_{condition_tag}_{run_id}.png")
        ...
        plt.title("Frequency dependence of reactance (capacitor)")

        result_text = (
            f"Estimated C = {C * 1e6:.5f} ± {u_C * 1e6:.5f} μF\n"
